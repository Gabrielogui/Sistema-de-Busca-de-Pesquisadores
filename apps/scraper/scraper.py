...
def obter_proximo_pendente(conn):
    cursor = conn.cursor()
    cursor.execute('''
...
def atualizar_status(conn, termo, status, tentativa):
    cursor = conn.cursor()
    cursor.execute('''
...
def limpar_texto(texto):
    """Limpeza GIGO de inconsistências geradas pela formatação do WebService governamental"""
    return texto.replace("¿", "-").replace("  ", " ").strip()

def get_campo_adjacente(soup, nome_campo):
    """Extrator lateral de rótulos PrimeFaces"""
...
def extrair_detalhes_rh(html_content):
    """ Mapeamento exclusivo da Tab 'Espelho do RH', base para cruzamento em IA Semantica! """
    soup = BeautifulSoup(html_content, 'html.parser')
    detalhes = {"lattes": "", "areas": [], "grupos": [], "linhas": []}

    # BURLADOR DO CAPTCHA LATTES (CORREÇÃO DA REGEX! O numero fica apos "espelhorh/")
    match = re.search(r'espelhorh/(\d{16})', html_content)
    if match: 
        detalhes["lattes"] = match.group(1)
    ...
    if areas_label:
        ...
        if div_control:
            for li in div_control.find_all('li'):
                detalhes["areas"].append(limpar_texto(li.get_text(strip=True)))

    tb_grupos = soup.find('tbody', id=re.compile(r'.*tblEspelhoRHGPAtuacao_data'))
    if tb_grupos:
        for tr in tb_grupos.find_all('tr'):
            tds = tr.find_all('td')
            if len(tds) >= 2 and tds[0].get('colspan') is None:
                detalhes["grupos"].append(limpar_texto(tds[0].get_text(strip=True)))
    ...
    if tb_linhas:
        for tr in tb_linhas.find_all('tr'):
            tds = tr.find_all('td')
            if len(tds) >= 2 and tds[0].get('colspan') is None:
                detalhes["linhas"].append(limpar_texto(tds[0].get_text(strip=True)))
...
def extrair_html_espelho(html_content, html_array_linhas_popups, membro_detalhes_map):
    soup = BeautifulSoup(html_content, 'html.parser')
    dados = {"id_dgp": "000000", "nome": "N/A", "repercussao": "", "area": "N/A", 
...
def extrair_dados_grupo(context, page, identificador_dgp):
    logger.info(f"Estabelecendo socket Lattes direto e Anti-Bot bypass! (ID DGP: {identificador_dgp})")
    try:
        ...
        logger.debug("Aguardando carregamento da Interface Web Central...")
        ...
        logger.debug(f"Interceptando Espelhos Auxiliares de RH (Pesquisador-Nodes). {total_membros} avaliacoes...")
        
        # VARREDURA DE MEMBROS E LATTES COM ACOMPANHAMENTO AO VIVO
        for index, tr in enumerate(linhas_tr, start=1):
            try:
                ...
                if len(tds) >= 2:
                    ...
                    if btn_rh.count() > 0:
                        logger.debug(f"[{index}/{total_membros}] Abrindo espelho cruzado para: {nome}")
                        ...
                        time.sleep(0.3) # Intervalo respeitoso do socket
                        
                        detalhes = extrair_detalhes_rh(aba_rh.content())
                        membro_detalhes_map[nome] = detalhes
                        
                        lattes_coletado = detalhes.get("lattes", "Não_Possui")
                        logger.debug(f"    └─> Extracao Limpa! Lattes associado: {lattes_coletado}")
                        
                        aba_rh.close()
                    else:
                        logger.debug(f"[{index}/{total_membros}] Alvo {nome} sem botões associados, ignorando espelho.")
            except Exception:
                pass
        
        # Abas de Linha De Pesquisa (Domínio secundário do Grupo)
        html_popups = []
        botoes_linha = page.locator("a[id*='idBtnVisualizarEspelhoLinhaPesquisa']").all()
        total_linhas = len(botoes_linha)
        logger.debug(f"Mapeamento humano concluido! Iniciando varredura das {total_linhas} Linhas de Pesquisa...")
        
        for idx, botao in enumerate(botoes_linha, start=1):
            logger.debug(f"[{idx}/{total_linhas}] Renderizando e Capturando HTML de Linha de Pesquisa...")
            with context.expect_page() as aba_linha_info: 
                botao.click()
        ...
        logger.debug("Componentes filhos e HTML Mestre coletados na totalidade. Executando Parser local.")
...
def salvar_xml(dados, identificador_nome):
    ...
    for m in dados.get("membros", []):
        ...
        # Multi-tabelas acopladas! O guia T.E.E.S validado!
    ...
    for linha in dados.get("linhas", []):
        ...
        ET.SubElement(lx, "NOME").text = linha.get('nome', '')
...
def executar_pipeline(grupos_especificos=None):
    ...
    if grupos_especificos:
        logger.info(f"Operacao Dedicada Invocada. Parametros em RAM e TXT listados: {grupos_especificos}")
        cursor = conn.cursor()
        for g in grupos_especificos: cursor.execute('INSERT OR IGNORE INTO fila_extracao (termo_busca, status) VALUES (?, "PENDENTE")', (g,))
        conn.commit()
    ...
    with sync_playwright() as p:
        ...
        while not _stop_requested:
            if grupos_especificos:
                ...
                item = obter_proximo_pendente(conn)
                if not item:
                    logger.info("Processamento Zero Defect Concluido - Queue Liberada.")
                    break
            ...
            try:
                ...
                atualizar_status(conn, termo, "CONCLUIDO", tentativas)
                logger.info(f"O motor gerou os DADOS TABULARES DA SPRINT 100% corretos: {termo} Finalizado.")
                
            except PlaywrightError as pe:
                logger.error(f"Erro em camada UI (JS). Time To Load Extrapolado em '{termo}'.", exc_info=True)
                atualizar_status(conn, termo, "ERRO", tentativas)
                if grupos_especificos: fila_manual.append({"termo": termo, "tentativas": tentativas})
            except Exception as ex:
                if "HTTP_429_RATE_LIMIT" in str(ex):
                    logger.error("Atividade Bloqueada temporariamente pela Nuvem Governo (429 Too Many R.) -> Hold On!.")
                    atualizar_status(conn, termo, "ERRO", tentativas)
                    if grupos_especificos: fila_manual.append({"termo": termo, "tentativas": tentativas})
                    for _ in range(60):
                    ...
                    logger.error(f"Exception não tratada no parse de variaveis nativas '{termo}'.", exc_info=True)
                    atualizar_status(conn, termo, "ERRO", tentativas)
                    if grupos_especificos: fila_manual.append({"termo": termo, "tentativas": tentativas})
                    time.sleep(2)
